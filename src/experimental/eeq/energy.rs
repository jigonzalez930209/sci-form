//! EEQ Energy and Gradients — E5.2
//!
//! Electrostatic energy from EEQ charges and analytical gradients.

use super::charges::{compute_eeq_charges, get_eeq_params, EeqConfig, erf_approx};
...
///
/// E_elec = sum_i chi_i * q_i + 0.5 * sum_{i,j} q_i * gamma(r_ij) * q_j
pub fn compute_eeq_energy(
    ...
    // One-electron term: chi_i * q_i
    for i in 0..n {
        let p = get_eeq_params(elements[i]);
        e_elec += p.chi * q[i];
    }

    // Two-electron term: 0.5 * sum q_i * gamma * q_j
    for i in 0..n {
    ...
    // Convert to kcal/mol (1 eV*e ≈ 23.06 kcal/mol for unit charge interactions)
...
///
/// Uses central finite differences: dE/dr_k ≈ (E(r+h) - E(r-h)) / 2h
...
mod tests {
    ...
    #[test]
    fn test_eeq_gradient_symmetric_molecule() {
        // Water: symmetric, so H gradients should be mirror images
        let elements = [8, 1, 1];
        let pos = [[0.0, 0.0, 0.0], [0.757, 0.586, 0.0], [-0.757, 0.586, 0.0]];
        let config = EeqConfig::default();
        let grad = compute_eeq_gradient(&elements, &pos, &config);
        // H1 and H2 x-gradients should be opposite
        assert!(
            (grad[1][0] + grad[2][0]).abs() < 0.5,
            "H gradients not symmetric: {:?} vs {:?}", grad[1], grad[2]
        );
    }
}
