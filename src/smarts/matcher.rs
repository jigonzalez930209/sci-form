...
use crate::graph::{Molecule, BondOrder, Hybridization};
...
pub fn substruct_match(mol: &Molecule, pattern: &SmartsPattern) -> Vec<Vec<usize>> {
    if pattern.atoms.is_empty() { return vec![]; }
...
fn substruct_match_inner(
    ...
    for start in 0..n_mol {
        mapping[0] = start;
        used[start] = true;
        if atom_matches(mol, NodeIndex::new(start), &pattern.atoms[0].query, ring_info) {
            backtrack(mol, pattern, &pat_adj, &mut mapping, &mut used, 1, ring_info, &mut results);
        }
        used[start] = false;
        mapping[0] = usize::MAX;
    }
...
fn backtrack(
    ...
    for nb in mol.graph.neighbors(mol_anchor) {
        ...
        // Check atom match
        if !atom_matches(mol, nb, &pattern.atoms[pat_atom].query, ring_info) { continue; }

        // Check bond match
        let edge = mol.graph.find_edge(mol_anchor, nb).unwrap();
        let mol_bond = &mol.graph[edge];
        if !bond_matches(mol_bond.order, &pattern.bonds[bond_idx].query, mol, mol_anchor, nb, ring_info) {
            continue;
        }
        ...
        for &(other_pat, bi) in &pat_adj[pat_atom] {
            if other_pat == connected_to { continue; }
            if mapping[other_pat] == usize::MAX { continue; }
            ...
            if let Some(e) = mol.graph.find_edge(nb, other_mol) {
                if !bond_matches(mol.graph[e].order, &pattern.bonds[bi].query, mol, nb, other_mol, ring_info) {
                    all_bonds_ok = false;
                    break;
                }
            } else {
        ...
        mapping[pat_atom] = ni;
        used[ni] = true;
        backtrack(mol, pattern, pat_adj, mapping, used, depth + 1, ring_info, results);
        used[ni] = false;
...
fn find_next_atom(
    ...
    for pat_idx in 0..pattern.atoms.len() {
        if mapping[pat_idx] != usize::MAX { continue; }
...
fn atom_matches(mol: &Molecule, atom: NodeIndex, query: &AtomQuery, ring_info: Option<&RingInfo>) -> bool {
    ...
    match query {
        ...
        AtomQuery::HeavyDegree(n) => {
            mol.graph.neighbors(atom).filter(|&nb| mol.graph[nb].element != 1).count() == *n as usize
        }
        ...
        AtomQuery::RingCount(n) => {
            if let Some(ri) = ring_info {
                let count = ri.rings.iter().filter(|r| r.contains(&atom.index())).count();
                count == *n as usize
            } else {
                *n == 0
            }
        ...
        AtomQuery::RingSize(n) => {
            if let Some(ri) = ring_info {
                ri.atom_ring_sizes[atom.index()].iter().any(|&s| s == *n as usize)
            } else {
                false
            }
        }
        AtomQuery::RingSizeRange(lo, hi) => {
            if let Some(ri) = ring_info {
                ri.atom_ring_sizes[atom.index()].iter().any(|&s| s >= *lo as usize && s <= *hi as usize)
            } else {
                false
            }
        }
        AtomQuery::RingSizeMin(lo) => {
            if let Some(ri) = ring_info {
                ri.atom_ring_sizes[atom.index()].iter().any(|&s| s >= *lo as usize)
            } else {
                false
            }
        }
        AtomQuery::FormalCharge(c) => a.formal_charge == *c,
        AtomQuery::Hybridization(n) => {
            match (n, &a.hybridization) {
                (1, Hybridization::SP) => true,
                (2, Hybridization::SP2) => true,
                (3, Hybridization::SP3) => true,
                _ => false,
            }
        }
...
fn bond_matches(
    ...
    match query {
        ...
        BondQuery::And(parts) => parts.iter().all(|q| bond_matches(order, q, mol, from, to, ring_info)),
        BondQuery::Not(inner) => !bond_matches(order, inner, mol, from, to, ring_info),
    }
}
...
fn compute_ring_info(mol: &Molecule) -> RingInfo {
    ...
    RingInfo { atom_in_ring, atom_ring_sizes, rings }
}
...
fn is_ring_bond(a: NodeIndex, b: NodeIndex, ring_info: &RingInfo) -> bool {
    ring_info.rings.iter().any(|ring| {
        let ai = a.index();
        let bi = b.index();
        if !ring.contains(&ai) || !ring.contains(&bi) { return false; }
...
fn count_ring_bonds(mol: &Molecule, atom: NodeIndex, ring_info: &RingInfo) -> usize {
    mol.graph.neighbors(atom).filter(|&nb| is_ring_bond(atom, nb, ring_info)).count()
}

fn is_aromatic_atom(mol: &Molecule, atom: NodeIndex) -> bool {
    mol.graph.edges(atom).any(|e| mol.graph[e.id()].order == BondOrder::Aromatic)
}

fn count_h(mol: &Molecule, atom: NodeIndex) -> usize {
    mol.graph.neighbors(atom).filter(|&nb| mol.graph[nb].element == 1).count()
}

fn pattern_needs_ring(pattern: &SmartsPattern) -> bool {
    for atom in &pattern.atoms {
        if query_needs_ring(&atom.query) { return true; }
    }
    for bond in &pattern.bonds {
        if bond_query_needs_ring(&bond.query) { return true; }
    }
    false
}
...
fn query_needs_ring(q: &AtomQuery) -> bool {
    match q {
        AtomQuery::InRing | AtomQuery::RingSize(_) |
        AtomQuery::RingSizeRange(..) | AtomQuery::RingSizeMin(_) |
        AtomQuery::RingBondCount(_) | AtomQuery::RingCount(_) => true,
