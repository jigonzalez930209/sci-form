...
fn main() {
    ...
    // Bonds to track
    let bond_pairs: Vec<(usize, usize)> = mol.graph.edge_references()
    ...
    let heavy: Vec<usize> = (0..n).filter(|&i| mol.graph[petgraph::graph::NodeIndex::new(i)].element != 1).collect();
    println!("\nAll heavy-atom bounds:");
    for a in 0..heavy.len() {
        for b in (a+1)..heavy.len() {
            let i = heavy[a]; let j = heavy[b];
    ...
    let csd_torsions: Vec<sci_form::forcefield::etkdg_3d::M6TorsionContrib> = {
        ...
        let all: std::collections::HashMap<String, Vec<serde_json::Value>> = serde_json::from_str(&data).unwrap();
        if let Some(torsions) = all.get(smi) {
            torsions.iter().map(|t| {
                let atoms: Vec<usize> = t["atoms"].as_array().unwrap().iter().map(|a| a.as_u64().unwrap() as usize).collect();
                let v: Vec<f64> = t["v"].as_array().unwrap().iter().map(|x| x.as_f64().unwrap()).collect();
                let signs: Vec<f64> = t["signs"].as_array().unwrap().iter().map(|x| x.as_f64().unwrap()).collect();
                sci_form::forcefield::etkdg_3d::M6TorsionContrib {
                    i: atoms[0], j: atoms[1], k: atoms[2], l: atoms[3],
    ...
    for t in &csd_torsions {
        println!("  [{},{},{},{}] V={:?} s={:?}", t.i, t.j, t.k, t.l, t.v, t.signs);
    }
    ...
    for c in ff.dist_12.iter().chain(ff.dist_13.iter()).chain(ff.dist_long.iter()) {
        if bond_pairs.contains(&(c.i, c.j)) || bond_pairs.contains(&(c.j, c.i)) {
            println!("  {}-{}: [{:.4}, {:.4}] k={}", c.i, c.j, c.min_len, c.max_len, c.k);
        }
    }
    ...
    println!("\nFinal coordinates:");
    for i in 0..n {
        println!("  atom {:2} ({}): {:.4} {:.4} {:.4}", i, mol.graph[petgraph::graph::NodeIndex::new(i)].element,
            refined_f32[(i,0)], refined_f32[(i,1)], refined_f32[(i,2)]);
    }
    ...
    println!("\nFF details:");
    println!("  dist constraints: {}", ff.dist_12.len() + ff.dist_13.len() + ff.dist_long.len());
    ...
    for tc in &ff.torsion_contribs {
        if tc.v.iter().any(|&x| x > 0.0) {
            println!("  torsion [{},{},{},{}] V={:?} s={:?}", tc.i, tc.j, tc.k, tc.l, tc.v, tc.signs);
        }
    }
    ...
    for &(i, j) in &bond_pairs {
        ...
        let lo = bounds[(j.max(i), j.min(i))];
        let hi = bounds[(j.min(i), j.max(i))];
        println!("  {}-{}: {:.4} (bounds: [{:.4}, {:.4}], diff: {:.4})", 
            i, j, d, lo, hi, d as f64 - (lo + hi) / 2.0);
    }
    ...
    for c in ff.dist_12.iter().chain(ff.dist_13.iter()).chain(ff.dist_long.iter()) {
        if c.k >= 99.0 {
            ...
            if (d as f64) < c.min_len || (d as f64) > c.max_len {
                println!("  {}-{}: d={:.4}, constraint=[{:.4},{:.4}], violation={:.4}", 
                    c.i, c.j, d, c.min_len, c.max_len,
                    if (d as f64) < c.min_len { c.min_len - d as f64 } else { d as f64 - c.max_len });
            }
        }
    }
}
