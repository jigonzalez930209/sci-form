...
fn main() {
    ...
    let heavy: Vec<usize> = (0..n).filter(|&i| {
    ...
    for ii in 0..heavy.len() {
        for jj in (ii+1)..heavy.len() {
            ...
            let lo = b[(j, i)];
            let hi = b[(i, j)];
            let elem_i = match mol.graph[ni].element { 6 => "C", 7 => "N", 8 => "O", _ => "?" };
            let elem_j = match mol.graph[nj].element { 6 => "C", 7 => "N", 8 => "O", _ => "?" };
            println!("  [{},{}] ({}-{}) [{:.3}, {:.3}]", i, j, elem_i, elem_j, lo, hi);
        }
    }
}
