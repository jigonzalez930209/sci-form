...
fn main() {
    ...
    for smi in &molecules {
        let mol = match Molecule::from_smiles(smi) {
            Ok(m) => m,
            Err(_) => { println!("{:30} PARSE_FAIL", smi); continue; }
        };
        ...
        for _iter in 0..max_iter {
            ...
            let mut coords = match coords_opt {
                Some(c) => c,
                None => { step_fails[0] += 1; continue; }
            };

            {
                let mut need = 1; let mut restarts = 0;
                while need != 0 && restarts < 5 {
                    need = sci_form::forcefield::bounds_ff::minimize_bfgs_rdkit(
                        &mut coords, &bounds, &chiral_sets, 400, 1e-3, 5.0, 0.1, 1.0,
                    );
                    restarts += 1;
                }
            }
            ...
            let e = sci_form::forcefield::bounds_ff::bounds_violation_energy_basin(
                &c32, &bounds, 5.0,
            ...
            if use_4d {
                ...
                while need != 0 && restarts < 5 {
                    need = sci_form::forcefield::bounds_ff::minimize_bfgs_rdkit(
                        &mut coords, &bounds, &chiral_sets, 200, 1e-3, 5.0, 1.0, 0.2,
                    );
                    restarts += 1;
                }
            }
...

            {
                ...
                let planarity_energy = sci_form::forcefield::etkdg_3d::planarity_check_energy(&ref_f32, &ff);
            ...
            println!(
                ...
                smi, _iter, step_fails[0], step_fails[1], step_fails[2],
                step_fails[3], step_fails[4], step_fails[5]
            );
            ok = true;
            break;
        }
        ...
        if !ok {
            println!(
                ...
                smi, max_iter, step_fails[0], step_fails[1], step_fails[2],
                step_fails[3], step_fails[4], step_fails[5]
            );
        }
    }
