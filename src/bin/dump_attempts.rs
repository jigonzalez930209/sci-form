...
fn main() {
    ...
    println!("chiral_sets: {}, use_4d: {}, embed_dim: {}", chiral_sets.len(), use_4d, embed_dim);
    ...
    for iter in 0..max_iterations {
        ...
        let mut coords = match coords_opt {
            Some(c) => c,
            None => {
                println!("attempt {}: embedding FAILED (rng: {} → {} → {})", 
        ...
        let energy_per_atom = {
            ...
            for i in 1..n {
                for j in 0..i {
                    ...
                    let ub2 = ub * ub;
                    let lb2 = lb * lb;
                    let val = if d2 > ub2 { d2 / ub2 - 1.0 }
                        else if d2 < lb2 { 2.0 * lb2 / (lb2 + d2) - 1.0 }
                        else { 0.0 };
                    if val > 0.0 { e += val * val; }
                }
            }
            e / n as f32
        };
        ...
        println!("attempt {}: PASSED all checks (rng: {} → {} → {})", 
            iter, rng_state_before, rng_after_dists, rng_after_coords);
        println!("  coords[0] = ({:.8}, {:.8}, {:.8})", coords[(0,0)], coords[(0,1)], coords[(0,2)]);
        break;
    }
}
