...
fn main() {
    ...
    for i in 0..mol.graph.node_count() {
        ...
        let nbs: Vec<_> = mol.graph.neighbors(petgraph::graph::NodeIndex::new(i))
            .map(|n| n.index())
            .collect();
        let elem = match a.element { 1 => "H", 6 => "C", 7 => "N", _ => "?" };
    ...
    match generate_3d_conformer_with_torsions(&mol, 42, &csd) {
        Ok(coords) => {
            ...
            for i in 0..mol.graph.node_count() {
                println!("Atom {}: ({:.4}, {:.4}, {:.4})", i, coords[(i,0)], coords[(i,1)], coords[(i,2)]);
            }

            // Compute torsion angles for CSD bonds
            fn torsion_angle(coords: &nalgebra::DMatrix<f32>, i: usize, j: usize, k: usize, l: usize) -> f32 {
                let p = |idx: usize| nalgebra::Vector3::new(coords[(idx,0)], coords[(idx,1)], coords[(idx,2)]);
                let b1 = p(j) - p(i);
                let b2 = p(k) - p(j);
                let b3 = p(l) - p(k);
                let n1 = b1.cross(&b2);
            ...
            println!("\nTorsion 0-1-2-3: {:.1}°", torsion_angle(&coords, 0, 1, 2, 3));
            println!("Torsion 4-3-2-1: {:.1}°", torsion_angle(&coords, 4, 3, 2, 1));
            ...
            for a in 0..n {
                for b in (a+1)..n {
                    let dr = ((rdkit[a].0-rdkit[b].0).powi(2)+(rdkit[a].1-rdkit[b].1).powi(2)+(rdkit[a].2-rdkit[b].2).powi(2)).sqrt();
                    let du = ((coords[(a,0)]-coords[(b,0)]).powi(2)+(coords[(a,1)]-coords[(b,1)]).powi(2)+(coords[(a,2)]-coords[(b,2)]).powi(2)).sqrt() as f64;
                    sq_sum += (dr-du).powi(2);
                    npairs += 1;
                }
            }
