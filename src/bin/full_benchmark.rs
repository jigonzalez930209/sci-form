...
fn main() {
    ...
    for mol in &bench_data.molecules {
        ...
        let mut our_mol = sci_form::graph::Molecule::from_smiles(&mol.smiles).expect("SMILES parsing failed");
        ...
        let mut coords3d_raw = sci_form::distgeom::generate_3d_coordinates(&metric);
    ...
    println!("Average time per molecule: {:.2} ms", rust_time_ms / bench_data.count as f64);
}
