...
fn build_mol_from_ref(r: &RefMolecule) -> Molecule {
    ...
    for b in &r.bonds {
        ...
        mol.add_bond(
            ...
            Bond { order, stereo: BondStereo::None },
        );
    }
    mol
}

fn build_csd_torsions(torsions: &[RefTorsion]) -> Vec<sci_form::forcefield::etkdg_3d::M6TorsionContrib> {
    torsions.iter().map(|t| {
        sci_form::forcefield::etkdg_3d::M6TorsionContrib {
            i: t.atoms[0], j: t.atoms[1], k: t.atoms[2], l: t.atoms[3],
            signs: [t.signs[0] as f64, t.signs[1] as f64, t.signs[2] as f64,
                    t.signs[3] as f64, t.signs[4] as f64, t.signs[5] as f64],
            v: [t.v[0], t.v[1], t.v[2],
                t.v[3], t.v[4], t.v[5]],
        }
    }).collect()
...
fn pairwise_rmsd(coords: &nalgebra::DMatrix<f32>, ref_atoms: &[RefAtom]) -> f32 {
    ...
    if npairs > 0 { (sq_sum / npairs as f64).sqrt() as f32 } else { 0.0 }
}
