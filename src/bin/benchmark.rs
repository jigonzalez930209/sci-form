...
fn main() {
    ...
    for mol in &bench_data.molecules {
        ...
        for bond in &mol.bonds {
            ...
            our_mol.add_bond(node_indices[bond.start], node_indices[bond.end], sci_form::graph::Bond {
                order,
        ...
        let coords3d = sci_form::distgeom::generate_3d_coordinates(&metric);
    ...
    println!("RDKit Total Time (Python): {:.2} ms", bench_data.rdkit_time_ms);
    println!("Sci-Form Total Time (Rust): {:.2} ms", rust_time_ms);
    println!("Sci-Form is {:.2}x faster!", bench_data.rdkit_time_ms / rust_time_ms);
}
