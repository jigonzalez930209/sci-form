...
fn assert_ts_near(energies: &[f64], expected: usize, tol: usize, label: &str) {
    ...
    assert!(dist <= tol, "{label}: TS en frame {ts}, esperado ≈ {expected} (tol={tol})");
}
...
fn print_energy_table(label: &str, energies: &[f64]) {
    ...
    for (i, e) in energies.iter().enumerate() {
        let mark = if i == MID { "  ← frame central (TS)" } else { "" };
...
fn r1_h2_double_well_21_frames() {
    ...
    println!("  {:>5}  {:>8}  {:>14}  {:>14}  {:>14}", "frame", "x", "E", "Ea_fwd", "Ea_rev");
    for (i, (&x, &e)) in xs.iter().zip(energies.iter()).enumerate() {
        ...
        println!("  {:>5}  {:>8.4}  {:>14.8}  {:>14.8}  {:>14.8}{}", i, x, e, ea_fwd, ea_rev, mark);
    }
    ...
    assert!((e_mid - 1.0).abs() < 1e-10, "V(0) debe valer exactamente 1.0, got {e_mid}");
    assert!(e_r < 1e-10 && e_p < 1e-10, "reactante y producto deben estar en 0");
    assert!((e_mid - e_r - (e_mid - e_p)).abs() < 1e-10, "reacción simétrica: Ea_fwd=Ea_rev");
}
...
fn r2_asymmetric_exothermic_21_frames() {
    ...
    let ts_idx  = energies.iter().enumerate()
        .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
        .map(|(i, _)| i).unwrap();
    ...
    assert!(ea_rev > ea_fwd, "exotérmico: Ea_rev={ea_rev:.4} debe > Ea_fwd={ea_fwd:.4}");
...
fn r3_muller_brown_2d_ts_search() {
    ...
    println!("  Reactante: ({:.2},{:.2})  Producto: ({:.2},{:.2})", start[0], start[1], end[0], end[1]);
    ...
    assert_eq!(energies.len(), N);
    assert!(ts_idx > 0 && ts_idx < N - 1, "TS no puede estar en extremos");
...
fn r4_proton_transfer_marcus_evb_21_frames() {
    ...
    println!("\n  {:>5}  {:>8}  {:>10}  {:>10}  {:>12}", "frame", "s", "ε_D", "ε_A", "E_EVB");
    for (i, &(s, e)) in data.iter().enumerate() {
        ...
        println!("  {:>5}  {:>8.4}  {:>10.4}  {:>10.4}  {:>12.6}{}", i, s, eps_d, eps_a, e, mark);
    }
    println!("\n  E(R)={e_r:.6}  E(TS@10)={e_mid:.6}  E(P)={e_p:.6}");
    println!("  Ea_fwd={ea_fwd:.6}  Ea_rev={ea_rev:.6}  |Ea_fwd-Ea_rev|={:.1e}", (ea_fwd - ea_rev).abs());
    ...
    assert!(asym < 1e-8, "EVB simétrico: |Ea_fwd-Ea_rev|={asym:.2e} debe ser < 1e-8");
}
...
fn r5_sn2_model_21_frames_plus_kinetics() {
    let xs: Vec<f64> = (0..N).map(|i| -1.5 + 3.0 * i as f64 / (N - 1) as f64).collect();
    ...
    #[cfg(feature = "alpha-kinetics")]
    {
        ...
        for (t, r) in temps.iter().zip(sweep.iter()) {
            println!("  {:>7.0}  {:>14.4e}  {:>14.4e}",
                t, r.forward_rate_s_inv, r.equilibrium_constant);
        }
        for w in sweep.windows(2) {
            assert!(w[1].forward_rate_s_inv >= w[0].forward_rate_s_inv,
        ...
        let config = MicrokineticNetworkConfig {
            ...
            total_time_s: 5e-2,
            n_frames: N,
            state: ThermodynamicState { temperature_k: 400.0, pressure_bar: 1.0 },
        };

        let trace = solve_microkinetic_network(&config).unwrap();
        println!("\n  ── Level 4: Red microcinética (21 frames temporales) ──");
        println!("  {:>10}  {:>12}  {:>14}  {:>10}", "t(s)", "[Reactante]", "[Complejo-TS]", "[Producto]");
        for f in &trace.frames {
            println!("  {:>10.3e}  {:>12.4}  {:>14.4}  {:>10.4}",
        ...
        assert!(!last.species[2].population.is_nan() && !last.species[2].population.is_infinite(),
            "concentración producto no debe ser NaN/inf");
        assert!(last.species[2].population > first.species[2].population,
            "Producto debe formarse a lo largo de la reacción");
        assert!(last.species[0].population < first.species[0].population,
...
fn level2_eht_h2_and_h2o_scan_21_frames() {
    ...
    let r_vals: Vec<f64> = (0..N).map(|i| 0.5 + 2.5 * i as f64 / (N - 1) as f64).collect();
    ...
    println!("  {:>5}  {:>8}  {:>14}  {:>10}", "frame", "r(Å)", "HOMO(eV)", "gap(eV)");
    for (i, (r, res)) in r_vals.iter().zip(eht_h2.iter()).enumerate() {
        let mark = if i == MID { "  ← frame 10" } else { "" };
        match res {
            Some((homo, gap)) => println!("  {:>5}  {:>8.4}  {:>14.6}  {:>10.6}{}", i, r, homo, gap, mark),
            None              => println!("  {:>5}  {:>8.4}  skipped{}", i, r, mark),
        }
    }

    // H₂O: θ = 90° → 180° en 21 frames (paralelo)
    let angles: Vec<f64> = (0..N).map(|i| 90.0 + 90.0 * i as f64 / (N - 1) as f64).collect();
    let r_oh = 0.957_f64;
    ...
    println!("  {:>5}  {:>8}  {:>12}  {:>10}  {:>8}", "frame", "θ(°)", "HOMO(eV)", "gap(eV)", "f+(O)");
    for (i, (theta, res)) in angles.iter().zip(eht_h2o.iter()).enumerate() {
        let mark = if i == MID { "  ← frame 10" } else { "" };
        match res {
            Some((homo, gap, fp)) => println!("  {:>5}  {:>8.2}  {:>12.6}  {:>10.6}  {:>8.4}{}", i, theta, homo, gap, fp, mark),
...
fn method_comparison_4_levels_h2_21_frames() {
    ...
    let l2: Vec<f64> = xs.par_iter().map(|&x| {
    ...
    // Level 3: PM3
    let l3: Vec<f64> = xs.par_iter().map(|&x| {
        let r = 0.5 + (x + 1.0) * 1.25;
    ...
    // Level 4: xTB
    let l4: Vec<f64> = xs.par_iter().map(|&x| {
        let r = 0.5 + (x + 1.0) * 1.25;
        let positions = vec![[0.0_f64, 0.0, 0.0], [r, 0.0, 0.0]];
    ...
    for i in 0..N {
        ...
        println!("  {:>5}  {:>8.4}  {:>10.4}  {:>10.4}  {:>10.4}  {:>10.4}{}",
                 i, r,
                 l1[i],
                 l2[i],
    ...
    if l2_valid.len() >= 3 {
        ...
        assert!(spread > 0.0, "EHT debe mostrar variación de energía: spread={spread}");
    ...
    let _: Vec<f64> = (0..N).into_par_iter().map(|i| {
...
fn neb_uff_h2o_bending_21_images() {
    ...
    let start_coords = vec![
        0.0, 0.0, 0.0,
        r_oh * half0.sin(),  r_oh * half0.cos(), 0.0,
        -r_oh * half0.sin(), r_oh * half0.cos(), 0.0,
    ];
    let end_coords = vec![
        0.0, 0.0, 0.0,
        r_oh * half1.sin(),  r_oh * half1.cos(), 0.0,
        -r_oh * half1.sin(), r_oh * half1.cos(), 0.0,
    ];

    match compute_simplified_neb_path("[H]O[H]", &start_coords, &end_coords, N, 5, 0.1, 0.001) {
        Ok(neb) => {
            let energies: Vec<f64> = neb.images.iter().map(|i| i.potential_energy_kcal_mol).collect();
            ...
            println!("  n_images={}, {}/{} energías finitas, TS@frame{}",
                neb.images.len(), e_finite.len(), N, ts_idx);
            println!("\n  {:>5}  {:>14}  {:>14}", "img", "E(kcal/mol)", "O-H dist");
            for img in &neb.images {
                let c  = &img.coords;
                let d1 = ((c[3]-c[0]).powi(2)+(c[4]-c[1]).powi(2)+(c[5]-c[2]).powi(2)).sqrt();
                ...
                if energies[img.index].is_finite() {
                    println!("  {:>5}  {:>14.4}  {:>14.4}{}", img.index, img.potential_energy_kcal_mol, d1, mark);
                }
            }
...
fn reactivity_fukui_descriptors_21_frames() {
    ...
    for (i, (theta, r)) in angles.iter().zip(results.iter()).enumerate() {
        ...
        match r {
            Some((homo, gap, fp, fm)) => {
                let dual = fp - fm;
                println!("  {:>5}  {:>8.2}  {:>12.6}  {:>10.6}  {:>8.4}  {:>8.4}  {:>10.4}{}",
...
fn gsm_three_reactions_21_nodes_mid_is_ts() {
    ...
    let potentials: Vec<(&str, Box<dyn Fn(&[f64]) -> f64 + Send + Sync>, [f64; 3], [f64; 3])> = vec![
        ("Doble pozo 1D", Box::new(|c: &[f64]| double_well(c[0])),   [-1., 0., 0.], [1., 0., 0.]),
        ("SN2 modelo 1D", Box::new(|c: &[f64]| sn2(c[0])),           [-1.5, 0., 0.], [1.5, 0., 0.]),
        ("EVB protón",    Box::new(|c: &[f64]| evb(c[0], 1.0, 0.15)), [-1., 0., 0.], [1., 0., 0.]),
    ];

    println!("\n═══ GSM: 3 reacciones × 21 nodos (paralelo) ═══");
    let results: Vec<_> = potentials.par_iter().map(|(name, efn, r, p)| {
        let result = find_transition_state(r, p, efn.as_ref(), &cfg);
        (name, result)
    }).collect();

    println!("  {:>22}  {:>8}  {:>8}  {:>8}  {:>10}  {:>10}",
             "Reacción", "n_nodes", "ts_idx", "MID", "Ea_fwd", "Ea_rev");
    for (name, res) in &results {
        println!("  {:>22}  {:>8}  {:>8}  {:>8}  {:>10.4}  {:>10.4}",
                 name, res.n_nodes, res.ts_node_index, res.n_nodes / 2,
                 res.activation_energy, res.reverse_barrier);
    }
    ...
    for (name, res) in &results {
        ...
        for (i, (coords, e)) in res.path_coords.iter().zip(res.path_energies.iter()).enumerate() {
    ...
    for (name, res) in &results {
        ...
        println!("  ✓ {name}: TS=frame{} MID={mid2} Δ={dist}", res.ts_node_index);
    }
}
