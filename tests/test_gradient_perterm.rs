...
fn torsion_only_energy(coords: &[f64], n: usize, ff: &sci_form::forcefield::etkdg_3d::Etkdg3DFF) -> f64 {
...
fn inversion_only_energy(coords: &[f64], n: usize, ff: &sci_form::forcefield::etkdg_3d::Etkdg3DFF) -> f64 {
    let c = |atom: usize, d: usize| -> f64 { coords[atom * 3 + d] };
    let mut energy = 0.0f64;
    for ic in &ff.inversion_contribs {
        ...
        let n_vec = [rji[1]*rjk[2]-rji[2]*rjk[1], rji[2]*rjk[0]-rji[0]*rjk[2], rji[0]*rjk[1]-rji[1]*rjk[0]];
        let n_len = (n_vec[0]*n_vec[0]+n_vec[1]*n_vec[1]+n_vec[2]*n_vec[2]).sqrt();
        let rjl_len = (rjl[0]*rjl[0]+rjl[1]*rjl[1]+rjl[2]*rjl[2]).sqrt();
        if n_len < 1e-8 || rjl_len < 1e-8 { continue; }
...
fn dist_only_energy(coords: &[f64], n: usize, ff: &sci_form::forcefield::etkdg_3d::Etkdg3DFF) -> f64 {
    let c = |atom: usize, d: usize| -> f64 { coords[atom * 3 + d] };
    let mut energy = 0.0f64;
    for dc in ff.dist_12.iter().chain(ff.dist_13.iter()).chain(ff.dist_long.iter()) {
        let dx = c(dc.i,0)-c(dc.j,0);
        let dy = c(dc.i,1)-c(dc.j,1);
        let dz = c(dc.i,2)-c(dc.j,2);
        let d = (dx*dx+dy*dy+dz*dz).sqrt();
        let term = if d < dc.min_len { d - dc.min_len } else if d > dc.max_len { d - dc.max_len } else { 0.0 };
...
fn angle_only_energy(coords: &[f64], n: usize, ff: &sci_form::forcefield::etkdg_3d::Etkdg3DFF) -> f64 {
    let c = |atom: usize, d: usize| -> f64 { coords[atom * 3 + d] };
    let mut energy = 0.0f64;
    for ac in &ff.angle_constraints {
        ...
        let cos_theta = ((r1[0]*r2[0]+r1[1]*r2[1]+r1[2]*r2[2]) / (l1*l2)).clamp(-1.0, 1.0);
        let theta_deg = cos_theta.acos() * 180.0 / std::f64::consts::PI;
        let term = if theta_deg < ac.min_deg { theta_deg - ac.min_deg }
                   else if theta_deg > ac.max_deg { theta_deg - ac.max_deg }
                   else { 0.0 };
...
fn test_gradient_perterm() {
    ...
    for &mol_idx in &test_indices {
        if mol_idx >= ref_mols.len() { continue; }
        ...
        println!("  FF: {} torsions, {} inversions, {} angles, {} dist_12, {} dist_13, {} dist_long",
            ff.torsion_contribs.len(), ff.inversion_contribs.len(),
            ff.angle_constraints.len(), ff.dist_12.len(), ff.dist_13.len(), ff.dist_long.len());

        // Check total gradient first
        let anal_grad = sci_form::forcefield::etkdg_3d::etkdg_3d_gradient_f64(&coords, n, &mol, &ff);
        ...
        println!("  TOTAL worst: component={} (atom {} {}) anal={:.8e} num={:.8e} rel_err={:.4e}",
        ...
        println!("  Sum-of-parts num: {:.8e} (torsion={:.4e} inv={:.4e} dist={:.4e} angle={:.4e})",
