...
fn test_grad_isolate() {
    ...
    for &mol_idx in &test_indices {
        ...
        let anal_full = sci_form::forcefield::etkdg_3d::etkdg_3d_gradient_f64(&coords, n, &mol, &ff_full);
        let num_full = numerical_gradient(&|c| sci_form::forcefield::etkdg_3d::etkdg_3d_energy_f64(c, n, &mol, &ff_full), &coords, eps);
        let mut worst_i = 0;
        ...
        println!("  FULL gradient worst: comp={} (atom {} {}) anal={:.8e} num={:.8e} rel={:.4e}",
        ...
        let term_names = ["torsions", "inversions", "dist_12", "angles", "dist_13", "dist_long"];
        ...
        for (ti, term_name) in term_names.iter().enumerate() {
            ...
            let ff_iso = sci_form::forcefield::etkdg_3d::Etkdg3DFF {
                torsion_contribs: if ti == 0 { ff_full.torsion_contribs.clone() } else { vec![] },
                inversion_contribs: if ti == 1 { ff_full.inversion_contribs.clone() } else { vec![] },
                dist_12: if ti == 2 { ff_full.dist_12.clone() } else { vec![] },
                angle_constraints: if ti == 3 { ff_full.angle_constraints.clone() } else { vec![] },
                dist_13: if ti == 4 { ff_full.dist_13.clone() } else { vec![] },
                dist_long: if ti == 5 { ff_full.dist_long.clone() } else { vec![] },
                oop_k: ff_full.oop_k,
            ...
            let anal = sci_form::forcefield::etkdg_3d::etkdg_3d_gradient_f64(&coords, n, &mol, &ff_iso);
            ...
            for i in 0..(n*3) {
                ...
                if rel > term_worst_rel { term_worst_rel = rel; term_worst_i = i; }
            }
            ...
            if term_worst_rel > 1e-3 {
                ...
                println!("  [OK]   {:12}: worst_rel={:.4e}  |  at global worst: A={:.4e} N={:.4e}",
