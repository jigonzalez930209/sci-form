...
fn run_with_timeout<F: FnOnce() + Send + 'static>(label: &str, f: F) -> Result<(), String> {
    ...
    match rx.recv_timeout(Duration::from_secs(TIMEOUT_SECS)) {
        ...
        Err(mpsc::RecvTimeoutError::Timeout) => {
            ...
            eprintln!("\n⏱  TIMEOUT: {} exceeded {}s (ran {:.1}s)", label, TIMEOUT_SECS, elapsed.as_secs_f64());
            Err(format!("timeout after {:.1}s", elapsed.as_secs_f64()))
        }
        Err(mpsc::RecvTimeoutError::Disconnected) => {
            let elapsed = start.elapsed();
            eprintln!("\n💥 PANIC: {} panicked after {:.1}s", label, elapsed.as_secs_f64());
...
fn exp2_scf_water() -> (
    ...
    let scf = sci_form::scf::scf_loop::run_scf(
        &sys,
        &sci_form::scf::scf_loop::ScfConfig::default(),
    );
    let basis = sci_form::scf::basis::BasisSet::sto3g(
        &elements, &pos_bohr,
    );
...
mod scf_comparison {
    ...
    fn bench_scf_h2o_production_vs_exp2() {
        timed_bench!("SCF H₂O prod vs exp2", {
            ...
            let t0 = Instant::now();
            let hf_result = sci_form::compute_hf3c(&elements, &positions, &sci_form::hf::HfConfig::default());
            let t_prod = t0.elapsed();

            let sys = sci_form::scf::types::MolecularSystem::from_angstrom(&elements, &positions, 0, 1);
            let t0 = Instant::now();
            let exp2 = sci_form::scf::scf_loop::run_scf(
                &sys, &sci_form::scf::scf_loop::ScfConfig::default(),
            );
            let t_exp2 = t0.elapsed();

            eprintln!("\n=== SCF H₂O: Production HF-3c vs Experimental_2 ===");
            eprintln!("Production:     {:.1} ms, converged={}", t_prod.as_secs_f64() * 1000.0,
                hf_result.as_ref().map_or(false, |r| r.converged));
            eprintln!("Experimental_2: {:.1} ms, converged={}", t_exp2.as_secs_f64() * 1000.0, exp2.converged);
            if let Ok(ref hf) = hf_result {
                eprintln!("Speedup:        {:.2}x", t_prod.as_secs_f64() / t_exp2.as_secs_f64());
                assert!(hf.converged);
            }
    ...
    fn bench_scf_ethanol_production_vs_exp2() {
        timed_bench!("SCF Ethanol prod vs exp2", {
            let (elements, positions) = ethanol_system();
            ...
            eprintln!("\n=== SCF Ethanol: Production vs Experimental_2 ===");
            eprintln!("Production:     {:.1} ms", t_prod.as_secs_f64() * 1000.0);
            eprintln!("Experimental_2: {:.1} ms", t_exp2.as_secs_f64() * 1000.0);
            if hf_result.is_ok() {
                eprintln!("Speedup:        {:.2}x", t_prod.as_secs_f64() / t_exp2.as_secs_f64());
            }
        });
    ...
    fn bench_scf_benzene_scaling() {
        timed_bench!("SCF Benzene exp2", {
            ...
            let sys = sci_form::scf::types::MolecularSystem::from_angstrom(&elements, &positions, 0, 1);
            let t0 = Instant::now();
            let result = sci_form::scf::scf_loop::run_scf(
                &sys, &sci_form::scf::scf_loop::ScfConfig::default(),
            );
            ...
            eprintln!("ERI O(N⁴) = O({}⁴) = {} quartets", result.n_basis, (result.n_basis as u64).pow(4));
...
mod eri_benchmarks {
    ...
    fn bench_eri_serial_vs_parallel() {
        timed_bench!("ERI serial vs parallel", {
            let (elements, positions) = ethanol_system();
            let sys = sci_form::scf::types::MolecularSystem::from_angstrom(&elements, &positions, 0, 1);
            let basis = sci_form::scf::basis::BasisSet::sto3g(
                &sys.atomic_numbers, &sys.positions_bohr,
            );

            let t0 = Instant::now();
            let eri_serial = sci_form::scf::two_electron::TwoElectronIntegrals::compute(&basis);
            let t_serial = t0.elapsed();

            let t0 = Instant::now();
            let eri_parallel = sci_form::scf::two_electron::TwoElectronIntegrals::compute_parallel(&basis);
            let t_par = t0.elapsed();

            let n = basis.n_basis;
            eprintln!("\n=== ERI Ethanol: Serial vs Parallel ===");
            eprintln!("N_basis:  {}", n);
            eprintln!("Serial:   {:.1} ms", t_serial.as_secs_f64() * 1000.0);
            eprintln!("Parallel: {:.1} ms", t_par.as_secs_f64() * 1000.0);
            eprintln!("Speedup:  {:.2}x", t_serial.as_secs_f64() / t_par.as_secs_f64());

            let max_diff = (0..n)
                .flat_map(|i| (0..n).flat_map(move |j| (0..n).flat_map(move |k| (0..n).map(move |l| (i, j, k, l)))))
                .map(|(i, j, k, l)| (eri_serial.get(i, j, k, l) - eri_parallel.get(i, j, k, l)).abs())
                .fold(0.0f64, f64::max);
            assert!(max_diff < 1e-12, "Serial/parallel mismatch: {:.2e}", max_diff);
            eprintln!("GPU_TIER: 1 — O(N⁴) embarrassingly parallel");
        });
    }

    #[test]
    fn bench_eri_benzene_scaling() {
        timed_bench!("ERI Benzene parallel", {
            let (elements, positions) = benzene_system();
            let sys = sci_form::scf::types::MolecularSystem::from_angstrom(&elements, &positions, 0, 1);
            let basis = sci_form::scf::basis::BasisSet::sto3g(
                &sys.atomic_numbers, &sys.positions_bohr,
            ...
            let t0 = Instant::now();
            let _eri = sci_form::scf::two_electron::TwoElectronIntegrals::compute_parallel(&basis);
            let elapsed = t0.elapsed();

            eprintln!("\n=== ERI Benzene (N_basis={}) ===", basis.n_basis);
            eprintln!("Time:   {:.1} ms", elapsed.as_secs_f64() * 1000.0);
...
mod one_electron_matrices {
    ...
    fn bench_overlap_matrix_h2o() {
        timed_bench!("Overlap matrix H₂O", {
            let (elements, _) = water_system();
            let pos_bohr = to_bohr(&water_system().1);
    ...
    fn bench_kinetic_matrix_h2o() {
        timed_bench!("Kinetic matrix H₂O", {
            ...
            for i in 0..n {
                for j in (i + 1)..n {
                    assert!((t_mat[(i, j)] - t_mat[(j, i)]).abs() < 1e-13, "T must be symmetric");
                }
            }
    ...
    fn bench_nuclear_matrix_h2o() {
        timed_bench!("Nuclear matrix H₂O", {
            ...
            let t0 = Instant::now();
            let v = sci_form::scf::nuclear_matrix::build_nuclear_matrix(&basis, &elements, &pos_bohr);
            let elapsed = t0.elapsed();
    ...
    fn bench_core_hamiltonian_h2o() {
        timed_bench!("Core Hamiltonian H₂O", {
            ...
            let t0 = Instant::now();
            let core = sci_form::scf::core_matrices::build_core_matrices(&basis, &elements, &pos_bohr);
            let elapsed = t0.elapsed();
            ...
            let e_nuc = sci_form::scf::core_matrices::nuclear_repulsion_energy(&elements, &pos_bohr);
...
mod fock_benchmarks {
    ...
    fn bench_fock_build_h2o() {
        timed_bench!("Fock build H₂O", {
            ...
            let core = sci_form::scf::core_matrices::build_core_matrices(
                &basis, &elements, &pos_bohr,
            );
            let eris = sci_form::scf::two_electron::TwoElectronIntegrals::compute(&basis);
            let n_occ = scf.n_electrons / 2;
            let density = sci_form::scf::density_matrix::build_density_matrix(
                &scf.mo_coefficients, n_occ,
            );

            let t0 = Instant::now();
            let fock = sci_form::scf::fock_matrix::build_fock_matrix(
                &core.core_hamiltonian, &density, &eris,
            );
            let t_fock = t0.elapsed();

            eprintln!("\n=== Fock Matrix Build H₂O ===");
            eprintln!("N_basis:  {}", basis.n_basis);
            eprintln!("Time:     {:.3} ms", t_fock.as_secs_f64() * 1000.0);
    ...
    fn bench_density_matrix_h2o() {
        timed_bench!("Density matrix H₂O", {
            ...
            let t0 = Instant::now();
            let p = sci_form::scf::density_matrix::build_density_matrix(
                &scf.mo_coefficients, n_occ,
            );
    ...
    fn bench_gamma_matrix_h2o() {
        timed_bench!("Gamma matrix H₂O", {
            let (elements, positions) = water_system();
            let pos_bohr = to_bohr(&positions);
            let eta: Vec<f64> = elements.iter().map(|&z| match z {
                1 => 0.4720,
                8 => 0.4991,
                _ => 0.3,
            }).collect();
...
mod mulliken_comparison {
    ...
    fn bench_mulliken_h2o() {
        timed_bench!("Mulliken H₂O", {
            ...
            if let Ok(ref p) = pop {
                eprintln!("Production: {:?}", p.mulliken_charges.iter().map(|x| format!("{:.4}", x)).collect::<Vec<_>>());
            }
            eprintln!("Exp2:       {:?}", scf.mulliken_charges.iter().map(|x| format!("{:.4}", x)).collect::<Vec<_>>());
...
mod visualization_comparison {
    ...
    fn bench_orbital_grid_h2o() {
        timed_bench!("Orbital grid H₂O", {
            ...
            let params = sci_form::gpu::orbital_grid::GridParams::from_molecule(&pos_bohr, 0.3, 5.0);
            let n_points = params.n_points();
            let homo_idx = scf.n_electrons / 2 - 1;

            let t0 = Instant::now();
            let _grid = sci_form::gpu::orbital_grid::evaluate_orbital(
                &basis, &scf.mo_coefficients, homo_idx, &params,
            );
            let elapsed = t0.elapsed();
            ...
            eprintln!("Time:        {:.1} ms", elapsed.as_secs_f64() * 1000.0);
            eprintln!("Points/sec:  {:.0}", n_points as f64 / elapsed.as_secs_f64());
    ...
    fn bench_marching_cubes_h2o() {
        timed_bench!("Marching cubes H₂O", {
            ...
            let params = sci_form::gpu::orbital_grid::GridParams::from_molecule(&pos_bohr, 0.3, 5.0);
            let homo_idx = scf.n_electrons / 2 - 1;
            let grid = sci_form::gpu::orbital_grid::evaluate_orbital(
                &basis, &scf.mo_coefficients, homo_idx, &params,
            );

            let t0 = Instant::now();
            let mesh = sci_form::gpu::marching_cubes::marching_cubes_cpu(
                &grid.values, &params, 0.02,
            );
            let elapsed = t0.elapsed();
    ...
    fn bench_isosurface_pipeline_h2o() {
        timed_bench!("Isosurface pipeline H₂O", {
            ...
            let config = sci_form::gpu::isosurface::IsosurfaceConfig {
                spacing: 0.3,
                padding: 5.0,
                isovalue: 0.02,
                both_lobes: true,
            ...
            let t0 = Instant::now();
            let iso = sci_form::gpu::isosurface::generate_orbital_isosurface(
                &basis, &scf, homo_idx, &pos_bohr, &config,
            );
            let elapsed = t0.elapsed();

            eprintln!("\n=== Isosurface Pipeline H₂O (grid → MC → mesh) ===");
            eprintln!("Total triangles: {}", iso.total_triangles);
            eprintln!("Pipeline time:   {:.1} ms", elapsed.as_secs_f64() * 1000.0);
...
mod gradient_benchmarks {
    ...
    fn bench_numerical_gradient_h2o() {
        timed_bench!("Numerical gradient H₂O", {
            let (elements, positions) = water_system();
            let sys = sci_form::scf::types::MolecularSystem::from_angstrom(&elements, &positions, 0, 1);
            let config = sci_form::scf::scf_loop::ScfConfig::default();

            let t0 = Instant::now();
            let grad = sci_form::scf::gradients::numerical_gradient(&sys, &config, 1e-3);
            let elapsed = t0.elapsed();
...
mod spectroscopy_benchmarks {
    ...
    fn bench_stda_uvvis_h2o() {
        timed_bench!("sTDA UV-Vis H₂O", {
            ...
            let config = sci_form::spectroscopy::StdaConfig::default();

            let t0 = Instant::now();
            let result = sci_form::spectroscopy::compute_stda(
                &scf, basis_to_atom, &pos_bohr, &config,
            );
            let elapsed = t0.elapsed();
            ...
            for (i, tr) in result.transitions.iter().take(3).enumerate() {
                eprintln!("  #{}: {:.2} eV ({:.1} nm), f={:.4}", i + 1, tr.energy_ev, tr.wavelength_nm, tr.oscillator_strength);
            }
    ...
    fn bench_giao_nmr_h2o() {
        timed_bench!("GIAO NMR H₂O", {
            ...
            eprintln!("Time:      {:.1} ms", elapsed.as_secs_f64() * 1000.0);
            for s in &shieldings {
                eprintln!("  Atom {}: Z={}, σ_iso={:.2} ppm, δ={:.2} ppm",
    ...
    fn bench_hessian_h2o() {
        timed_bench!("Hessian H₂O", {
            let (elements, positions) = water_system();
            let sys = sci_form::scf::types::MolecularSystem::from_angstrom(&elements, &positions, 0, 1);
            let scf_config = sci_form::scf::scf_loop::ScfConfig::default();

            let t0 = Instant::now();
            let hess = sci_form::spectroscopy::hessian::compute_hessian(
                &sys, &scf_config, 1e-3, 5e-3,
            );
            let elapsed = t0.elapsed();

            eprintln!("\n=== Hessian H₂O (9×9, 6N gradient evals) ===");
            eprintln!("Time:         {:.1} ms", elapsed.as_secs_f64() * 1000.0);
            eprintln!("Frequencies:  {:?}", hess.frequencies.iter().map(|f| format!("{:.1} cm⁻¹", f)).collect::<Vec<_>>());
    ...
    fn bench_ir_h2o() {
        timed_bench!("IR spectrum H₂O", {
            ...
            eprintln!("\n=== IR Spectrum H₂O ===");
            eprintln!("Time after Hessian: {:.1} ms", elapsed.as_secs_f64() * 1000.0);
    ...
    fn bench_eeq_gpu_potential() {
        timed_bench!("EEQ charges benzene", {
            ...
            let result = sci_form::charges_eeq::compute_eeq_charges(
                &elements, &pos_bohr, &sci_form::charges_eeq::EeqConfig::default(),
            );
            let elapsed = t0.elapsed();

            eprintln!("\n=== EEQ Charges Benzene (N={}) ===", elements.len());
            eprintln!("Time:    {:.3} ms", elapsed.as_secs_f64() * 1000.0);
            eprintln!("Charges: {:?}", result.charges.iter().map(|x| format!("{:.3}", x)).collect::<Vec<_>>());
...
mod gpu_candidate_d4 {
    ...
    fn bench_d4_gpu_potential() {
        timed_bench!("D4 dispersion benzene", {
            ...
            let config = sci_form::dispersion::D4Config { three_body: true, ..Default::default() };

            let t0 = Instant::now();
            let result = sci_form::dispersion::compute_d4_energy(&elements, &pos_bohr, &config);
            let elapsed = t0.elapsed();

            eprintln!("\n=== D4 Dispersion Benzene (N={}) ===", elements.len());
            eprintln!("Time:      {:.3} ms", elapsed.as_secs_f64() * 1000.0);
            eprintln!("E_2body:   {:.6} Hartree", result.e2_body);
            eprintln!("E_3body:   {:.6} Hartree", result.e3_body);
    ...
    fn bench_kpm_gpu_potential() {
        timed_bench!("KPM DOS", {
            ...
            let config = sci_form::beta::kpm::KpmConfig {
                order: 100, n_vectors: 50, seed: 42, temperature: 0.1,
            };

            let t0 = Instant::now();
            let _dos = sci_form::beta::kpm::compute_kpm_dos(&h, &config, e_min, e_max, 200);
            let elapsed = t0.elapsed();
...
mod gpu_candidate_alpb {
    ...
    fn bench_alpb_gpu_potential() {
        timed_bench!("ALPB Born radii benzene", {
            ...
            eprintln!("Time:  {:.3} ms", elapsed.as_secs_f64() * 1000.0);
            eprintln!("Radii: {:?}", radii.radii.iter().map(|x| format!("{:.3}", x)).collect::<Vec<_>>());
...
mod gpu_candidate_cpm {
    ...
    fn bench_cpm_gpu_potential() {
        timed_bench!("CPM charges benzene", {
            let (elements, positions) = benzene_system();
            let pos_bohr = to_bohr(&positions);

            let config = sci_form::beta::cpm::CpmConfig {
                mu_ev: -4.44, dielectric: 1.0, max_iter: 200, charge_tol: 1e-8,
            };

            let t0 = Instant::now();
            let result = sci_form::beta::cpm::compute_cpm_charges(&elements, &pos_bohr, &config);
            let elapsed = t0.elapsed();

            eprintln!("\n=== CPM Charges Benzene (N={}) ===", elements.len());
            eprintln!("Time:       {:.3} ms", elapsed.as_secs_f64() * 1000.0);
            eprintln!("Converged:  {}", result.converged);
    ...
    fn bench_randnla_nystrom() {
        timed_bench!("RandNLA Nyström", {
            ...
            let (eigenvalues, _eigenvectors, info) = sci_form::beta::rand_nla::solve_eht_randnla(
                &core.core_hamiltonian, &core.overlap, &config,
            );
            let elapsed = t0.elapsed();
            ...
            eprintln!("Eigenvalues: {:?}", eigenvalues.iter().map(|x| format!("{:.4}", x)).collect::<Vec<_>>());
...
mod gpu_candidate_sdr {
    ...
    fn bench_sdr_alternating_projections() {
        timed_bench!("SDR alternating projections", {
            ...
            let t0 = Instant::now();
            let (gram, conv) = sci_form::alpha::sdr::alternating_projections(
                &x0, &distance_pairs, &config,
            );
            let elapsed = t0.elapsed();
...
mod gpu_candidate_cga {
    ...
    fn bench_cga_motor_application() {
        timed_bench!("CGA motor application", {
            ...
            let t0 = Instant::now();
            let _new_coords = sci_form::alpha::cga::apply_motor_to_subtree(&coords, &subtree, &motor);
            let elapsed = t0.elapsed();

            eprintln!("\n=== CGA Motor Application (N={} atoms, {} subtree) ===", n_atoms, subtree.len());
            eprintln!("Time:   {:.3} ms", elapsed.as_secs_f64() * 1000.0);
    ...
    fn bench_cga_torsion_refinement() {
        timed_bench!("CGA torsion refinement", {
            ...
            let t0 = Instant::now();
            let conformers = sci_form::alpha::cga::refine_torsion_cga(
                &coords, 1, 2, &subtree, 12,
            );
            let elapsed = t0.elapsed();
