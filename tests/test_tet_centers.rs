...
fn test_tetrahedral_centers() {
    ...
    use sci_form::distgeom::validation::{identify_tetrahedral_centers, find_sssr_pub};
    ...
    for smi in &smiles {
        ...
        println!("{}: rings={:?}, {} tetrahedral centers", smi, rings, centers.len());
    }
}
...
fn test_bounds_comparison() {
    ...
    let bounds = {
        ...
        let mut b = raw;
        if triangle_smooth_tol(&mut b, 0.05) { b }
        else {
    ...
    for i in 0..7 {
        for j in (i+1)..7 {
            ...
            let ub = bounds[(i, j)];
            let lb = bounds[(j, i)];
            println!("  ({},{}) lb={:.4}  ub={:.4}  range={:.4}", i, j, lb, ub, ub - lb);
        }
    }
}
