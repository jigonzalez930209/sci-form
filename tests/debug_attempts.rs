...
fn debug_attempt_tracking() {
    ...
    for (smiles, seed) in test_cases {
        ...
        for iter in 0..max_iter {
            ...
            let coords4d_opt =
                sci_form::distgeom::compute_initial_coords_rdkit(&mut rng, &dists, 4);
            ...
            match coords4d_opt {
                Some(mut coords4d) => {
                    ...
                    let mut energy = sci_form::forcefield::bounds_ff::bounds_violation_energy_basin(
                        &coords4d_f32,
                        &bounds,
                        5.0,
                    );
                    if !chiral_sets.is_empty() {
                        energy += sci_form::forcefield::bounds_ff::chiral_violation_energy(
                            &coords4d_f32,
                            &chiral_sets,
                        );
                    }
                    ...
                    if !chiral_sets.is_empty()
                        && !sci_form::distgeom::check_chiral_centers(&coords4d_f64, &chiral_sets)
                    {
                    ...
                    println!(
                        "  Iter {}: SUCCESS (E/atom={:.6}, {} BFGS passes)",
                        iter, e_per_atom, bfgs_passes
                    );
                    break;
                }
                None => {
