...
fn test_compare_bounds_matrix() {
    ...
    println!("Triangle smooth: {}", if smooth_ok { "OK" } else { "FAILED" });
    ...
    let data_start = if major >= 2 { 12 + header_len } else { 10 + header_len };
    ...
    for i in 0..n {
        for j in (i+1)..n {
            ...
            if ub_diff > 1e-10 { ub_diffs += 1; }
            if lb_diff > 1e-10 { lb_diffs += 1; }
            if ub_diff > max_ub_diff { max_ub_diff = ub_diff; }
            if lb_diff > max_lb_diff { max_lb_diff = lb_diff; }
            total_pairs += 1;
    ...
    for j in 1..std::cmp::min(n, 20) {
        println!("  bm[0,{:2}] = {:.10} (UB), bm[{:2},0] = {:.10} (LB)",
            j, bounds[(0, j)], j, bounds[(j, 0)]);
    }
}
