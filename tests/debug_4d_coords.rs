...
fn debug_4d_coords_mol34() {
    ...
    for i in 0..n {
        ...
        if elem != 1 {
            println!("  INIT atom {} (elem={}): ({:.10}, {:.10}, {:.10}, {:.10})", 
                i, elem, coords4d[(i,0)], coords4d[(i,1)], coords4d[(i,2)], coords4d[(i,3)]);
        }
    }
    ...
    for i in 0..n {
        let elem = mol.graph[petgraph::graph::NodeIndex::new(i)].element;
        if elem != 1 {
            println!("  POST atom {} (elem={}): ({:.10}, {:.10}, {:.10}, 4d={:.10})", 
                i, elem, coords4d[(i,0)], coords4d[(i,1)], coords4d[(i,2)], coords4d[(i,3)]);
        }
    }
    ...
    for i in 0..n {
        let elem = mol.graph[petgraph::graph::NodeIndex::new(i)].element;
        if elem != 1 {
            println!("  atom {} (elem={}): ({:.8}, {:.8}, {:.8})", 
                i, elem, coords3d[(i,0)], coords3d[(i,1)], coords3d[(i,2)]);
        }
    }
    ...
    for a in 0..heavy_idxs.len() {
        for b in (a+1)..heavy_idxs.len() {
            ...
            let our_d = ((coords3d[(i,0)] - coords3d[(j,0)]).powi(2) 
                ...
                + (coords3d[(i,2)] - coords3d[(j,2)]).powi(2)).sqrt();
            let rdkit_d = ((rdkit_dg_heavy[a].0 - rdkit_dg_heavy[b].0).powi(2)
                + (rdkit_dg_heavy[a].1 - rdkit_dg_heavy[b].1).powi(2)
                + (rdkit_dg_heavy[a].2 - rdkit_dg_heavy[b].2).powi(2)).sqrt();
            let diff = (our_d - rdkit_d).abs();
            if diff > 0.01 {
                println!("  Heavy pair ({},{}) = atoms ({},{}): our={:.6} rdkit={:.6} diff={:.6}",
    ...
    println!("\nPairwise distance RMSD (heavy atoms, 4D→3D vs RDKit pure DG): {:.6}", rmsd);
    ...
    for i in 0..n {
        for j in (i+1)..n {
            ...
            let rdkit_d = ((rdkit_dg_all[i].0 - rdkit_dg_all[j].0).powi(2)
                ...
                + (rdkit_dg_all[i].2 - rdkit_dg_all[j].2).powi(2)).sqrt();
            sq_sum_all += (our_d - rdkit_d).powi(2);
            npairs_all += 1;
        }
    }
    let rmsd_all = (sq_sum_all / npairs_all as f64).sqrt();
    println!("Pairwise distance RMSD (all atoms, 4D→3D vs RDKit pure DG): {:.6}", rmsd_all);
}
...
fn debug_ethane_coords() {
    ...
    println!("\nInitial 4D coords:");
    for i in 0..n {
        println!("  atom {}: ({:.12}, {:.12}, {:.12}, {:.12})", 
            i, coords4d[(i,0)], coords4d[(i,1)], coords4d[(i,2)], coords4d[(i,3)]);
    }
    ...
    println!("\n3D coords (pre-ETKDG):");
    for i in 0..n {
        println!("  atom {}: ({:.12}, {:.12}, {:.12})", 
            i, coords3d_f64[(i,0)], coords3d_f64[(i,1)], coords3d_f64[(i,2)]);
    }
    ...
    for i in 0..n {
        ...
        println!("  atom {}: ours=({:.10}, {:.10}, {:.10}) rdkit=({:.10}, {:.10}, {:.10}) dist={:.8}",
            i, final_coords[(i,0)], final_coords[(i,1)], final_coords[(i,2)],
            rdkit_ref[i][0], rdkit_ref[i][1], rdkit_ref[i][2], d);
    }
    ...
    for i in 0..n {
        for j in (i+1)..n {
            let our_d = (((final_coords[(i,0)] - final_coords[(j,0)]) as f64).powi(2)
                ...
                + ((final_coords[(i,2)] - final_coords[(j,2)]) as f64).powi(2)).sqrt();
            let rdkit_d = ((rdkit_ref[i][0] - rdkit_ref[j][0]).powi(2)
                + (rdkit_ref[i][1] - rdkit_ref[j][1]).powi(2)
                + (rdkit_ref[i][2] - rdkit_ref[j][2]).powi(2)).sqrt();
            sq_sum += (our_d - rdkit_d).powi(2);
            npairs += 1;
        }
    }
