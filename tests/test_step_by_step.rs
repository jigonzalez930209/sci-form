...
#[derive(Deserialize)]
struct RefTorsion { atoms: Vec<usize>, v: Vec<f64>, signs: Vec<f64> }
#[derive(Deserialize)]
struct RefMolecule {
    smiles: String,
    atoms: Vec<RefAtom>,
...
fn build_mol_from_ref(ref_mol: &RefMolecule) -> sci_form::graph::Molecule {
    ...
    for atom in &ref_mol.atoms {
        ...
        let new_atom = sci_form::graph::Atom {
            element: atom.element, position: nalgebra::Vector3::zeros(), charge: 0.0,
            formal_charge: atom.formal_charge, hybridization,
    ...
    for bond in &ref_mol.bonds {
        ...
        mol.add_bond(node_indices[bond.start], node_indices[bond.end],
            sci_form::graph::Bond { order, stereo: sci_form::graph::BondStereo::None });
    }
    mol
}

fn build_csd_torsions(ref_torsions: &[RefTorsion]) -> Vec<sci_form::forcefield::etkdg_3d::M6TorsionContrib> {
    ref_torsions.iter().filter_map(|t| {
...
fn test_step_by_step_trace() {
    ...
    eprintln!("Step 2: chiral_sets={}, tet_centers={}, use_4d={}, embed_dim={}",
        chiral_sets.len(), tet_centers.len(), use_4d, embed_dim);
    ...
    for iter in 0..max_iterations {
        ...
        let coords_opt = sci_form::distgeom::compute_initial_coords_rdkit(&mut rng, &dists, embed_dim);
            ...
            for a in 0..n {
                for b in (a+1)..n {
                    let dr = ((ref_mol.atoms[a].x - ref_mol.atoms[b].x).powi(2) +
                              ...
                              (ref_mol.atoms[a].z - ref_mol.atoms[b].z).powi(2)).sqrt();
                    let du = ((coords[(a,0)] - coords[(b,0)]).powi(2) +
                              (coords[(a,1)] - coords[(b,1)]).powi(2) +
                              (coords[(a,2)] - coords[(b,2)]).powi(2)).sqrt();
                    sq_sum += (dr - du).powi(2);
                    npairs += 1;
                }
            }
        ...
        eprintln!("  attempt {} → embedding OK (rng_before={}, initial_RMSD={:.4})", iter, rng_state_before, initial_rmsd);
        ...
        if initial_energy > error_tol {
            ...
            let post_energy = sci_form::conformer::compute_total_bounds_energy_f64(
                &coords, &bounds, &chiral_sets, basin_thresh, 0.1, 1.0);
            eprintln!("    bounds_ff_1: {} BFGS calls, final_energy={:.6}", bfgs_calls, post_energy);
        }
            ...
            for a in 0..n {
                for b in (a+1)..n {
                    ...
                    let du = ((coords3d[(a,0)] - coords3d[(b,0)]).powi(2) +
                              ...
                              (coords3d[(a,2)] - coords3d[(b,2)]).powi(2)).sqrt();
                    sq_sum += (dr - du).powi(2);
                    npairs += 1;
                }
            }
        ...
        let ff = sci_form::forcefield::etkdg_3d::build_etkdg_3d_ff_with_torsions(
            &mol, &coords3d, &bounds, &csd_torsions);
        
        let flat_f64: Vec<f64> = {
            let mut flat = vec![0.0f64; n * 3];
            for a in 0..n { flat[a*3] = coords3d[(a,0)]; flat[a*3+1] = coords3d[(a,1)]; flat[a*3+2] = coords3d[(a,2)]; }
            flat
        };
            ...
            for a in 0..n {
                for b in (a+1)..n {
                    ...
                    let du = ((refined[(a,0)] - refined[(b,0)]).powi(2) +
                              ...
                              (refined[(a,2)] - refined[(b,2)]).powi(2)).sqrt();
                    sq_sum += (dr - du).powi(2);
                    npairs += 1;
                }
            }
        ...
        eprintln!("    FINAL RMSD={:.4} (initial={:.4}, post_bounds={:.4})", final_rmsd, initial_rmsd, post_bounds_rmsd);
        ...
        let flat_refined: Vec<f64> = {
            ...
            for a in 0..nr { flat[a*3] = refined[(a,0)]; flat[a*3+1] = refined[(a,1)]; flat[a*3+2] = refined[(a,2)]; }
            flat
        };
        let plan_e = sci_form::forcefield::etkdg_3d::planarity_check_energy_f64(&flat_refined, n, &ff);
        if plan_e > n_improper as f64 * 0.7 {
            eprintln!("    REJECT: planarity (energy={:.4} > {:.4})", plan_e, n_improper as f64 * 0.7);
            continue;
        }
    ...
    eprintln!("FAILED: no valid conformer after {} attempts", max_iterations);
}
