...
use sci_form::distgeom::{MinstdRand, pick_rdkit_distances, compute_initial_coords_rdkit,
...
fn test_debug_bounds() {
    ...
    let bounds = {
        ...
        let mut b = raw;
        if distgeom::triangle_smooth_tol(&mut b, 0.05) { b }
        else {
            let raw2 = distgeom::calculate_bounds_matrix_opts(&mol, false);
            let mut b2 = raw2.clone();
            if distgeom::triangle_smooth_tol(&mut b2, 0.0) { b2 }
            else { let mut b3 = raw2; distgeom::triangle_smooth_tol(&mut b3, 0.05); b3 }
        }
    };
    ...
    // 1st BFGS: chiral_w=1.0, 4d_w=0.1
    {
        ...
        let mut pass = 0;
        while nm != 0 {
            nm = minimize_bfgs_rdkit(&mut coords4d, &bounds, &chiral_sets, 400, 1e-4, 5.0, 0.1, 1.0);
            pass += 1;
        }
    ...
    // 2nd BFGS: chiral_w=0.2, 4d_w=1.0
    {
        ...
        let mut pass = 0;
        while nm != 0 {
            nm = minimize_bfgs_rdkit(&mut coords4d, &bounds, &chiral_sets, 200, 1e-4, 5.0, 1.0, 0.2);
            pass += 1;
        }
